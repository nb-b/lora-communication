...
class sx126x:
    ...
    def receive(self):
        if self.ser.inWaiting() > 0:
            ...
            # Add them to persistent buffer
            self.rx_buffer += data
            print("receive something")
    
            # Process every complete packet
            while b'break' in self.rx_buffer:
    
                # Extract ONE packet
                raw_packet, self.rx_buffer = self.rx_buffer.split(
                    b'break',
                    1
                )
    
                try:
                    message = raw_packet.decode('utf-8').strip()
                except UnicodeDecodeError:
                    print("Invalid UTF-8 packet:", raw_packet.hex())
                    continue
                ...
                # ==============================
                # TYPE|PACKET_ID|FROM|ORIGIN|DATA
                # ==============================
                parts = message.split(':')
                ...
                packet_type = parts[0]
                packet_id = parts[1]
                packet_nodeA = parts[2]
                packet_dataA = parts[3]
                packet_nodeB = parts[4]
                packet_dataB = parts[5]
                packet_nodeC = parts[6]
                packet_dataC = parts[7]
                
                
    
                # Pi only accepts DATA forwarded by A
                if packet_type == "DATA" and packet_nodeA == "A":
                    ...
                    print("Node A:", packet_nodeA, " ", packet_dataA)
                    print("Node B:", packet_nodeB, " ", packet_dataB)
                    print("Node C:", packet_nodeC, " ", packet_dataC)
                    print("Node D:", packet_nodeD, " ", packet_dataD)
